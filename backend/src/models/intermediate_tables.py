...
class ProjectMembership(SQLModel, table=True):
    ...
    project_id: int | None = Field(default=None, foreign_key="projects.id", primary_key=True, ondelete="CASCADE")
...
class IssueLabel(SQLModel, table=True):
    ...
    issue_id: int | None = Field(default=None, foreign_key="issues.id", primary_key=True, ondelete="CASCADE")
    label_id: int | None = Field(default=None, foreign_key="labels.id", primary_key=True, ondelete="CASCADE")
    # Relationships
